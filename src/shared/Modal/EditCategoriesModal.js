...
const EditCategoriesModal = ({ setVisibleEdit, data, setData }) => {
  //contex
  ...
  let handleCategory = {
    ...
    submitCategory: async () => {
      if (!category?.name) {
        ...
        let getData = await CreateEditCategory(category);
        if (getData?.status) {
          let allCategories = [...data?.data];
          allCategories[data?.selctedData?.index].name = category?.name;
          setData({
            ...data,
            data: allCategories,
          });
    ...
    <div className="modal">
      <div className="modal-content">
        ...
        <div className="modal-body mt-3">
          <h2 className="text-xl font-bold">Editing Task Category</h2>
